trigger Glxy_IBAA_Monthly_Fields on Glxy_IntraBudgetActivityAuthorization__c (before insert, before update) {
    
    Set<Id> leaseIds = new Set<Id>();

    for(Glxy_IntraBudgetActivityAuthorization__c ibaa: trigger.new) {
        leaseIds.add(ibaa.LeaseNumber__c);
    }
    ...
    for (Glxy_IntraBudgetActivityAuthorization__c curObject: trigger.new) {
        // Ensure EffectiveDate is populated
        if (curObject.EffectiveDate__c != null && !curObject.Overwrite__c) {
            ...
            if ((noaOrParkSqFt + iaSqFt) == 0) {
                monIA = 0;
                monNOA = curObject.AuthorizedAmount__c;
            } else if (noaOrParkSqFt == 0) {
                monNOA = 0;
                monIA = curObject.AuthorizedAmount__c/(noaOrParkSqFt + iaSqFt ) * iaSqFt;
            } else if (iaSqFt == 0 ) {
                monIA = 0;
                monNOA = curObject.AuthorizedAmount__c/(noaOrParkSqFt + iaSqFt) * noaOrParkSqFt;
            } else {
                monIA = curObject.AuthorizedAmount__c/(noaOrParkSqFt + iaSqFt ) * iaSqFt;
                monNOA = curObject.AuthorizedAmount__c/(noaOrParkSqFt + iaSqFt) * noaOrParkSqFt;
            }
