trigger GLXY_LumpSumTrigger on Glxy_LumpSum__c (before delete, before insert, after insert, before update) {
	
	List<GLXY_LumpSumObligations__c> objLSObligationsByFY = New List<GLXY_LumpSumObligations__c>();
	List<Glxy_LumpSum__c> objUpdateComments = New List<Glxy_LumpSum__c>();
	...
	if(Trigger.isBefore){
		if (Trigger.isDelete) {
			...
			string RexusRecordType=Schema.SObjectType.Glxy_LumpSum__c.getRecordTypeInfosByName().get('Rexus').getRecordTypeId();
			system.debug(RexusRecordType);
			for(Glxy_LumpSum__c pi:trigger.old ){
			...
			for(Glxy_LumpSum__c ls: trigger.new){
		       leaseIds.add(ls.LeaseNumber__c);
		    ...
		    Map<Id,Glxy_Lease__c> leases = new Map<Id,Glxy_Lease__c> ([Select id,
		                                  ParkingOnly__c,  BaseStructuredParkingSpaces__c, BaseSurfaceParkingSpaces__c,
		                                  IAConverstionDate__c, Total_NOA_Sq_feet__c, Total_IA_Sq_feet__c,
		                                  BaseAnnualRent__c, BaseLeaseEffectiveDate__c, IAConverstionFY__c
		                                  from Glxy_Lease__c where id IN :leaseIds]);
		    
		    //Bulkify calculations
		    for (Glxy_LumpSum__c curObject: trigger.new) {
		        ...
		        if (curObject.EffectiveDate__c != null && curObject.ExpirationDate__c != null) {
		            ...
		            if(leases.containsKey(curObject.LeaseNumber__c)) {
		                ...
		                IANOAConversionFY = Integer.valueOf(lease.IAConverstionFY__c);
		                ...
		                MonthlyRentOtherMonths = lease.BaseAnnualRent__c/12;
		                MonthLeaseEffective  = lease.BaseLeaseEffectiveDate__c.month();
		                YearLeaseEffective = lease.BaseLeaseEffectiveDate__c.Month() > 9? lease.BaseLeaseEffectiveDate__c.Year()+ 1 : lease.BaseLeaseEffectiveDate__c.Year();
		                DaysinLeaseEffectiveMonth = date.daysInMonth(lease.BaseLeaseEffectiveDate__c.year(), lease.BaseLeaseEffectiveDate__c.month());
		                DaysProratedEffectiveMonth = lease.BaseLeaseEffectiveDate__c.daysBetween(date.newInstance(lease.BaseLeaseEffectiveDate__c.year(), lease.BaseLeaseEffectiveDate__c.month(), DaysinLeaseEffectiveMonth)) + 1;
		            ...
		            for (Integer intLoopFY = calcFiscalYr; intLoopFY <= intLoopEndYear; intLoopFY ++){ //FY Loop Starts here
			            ...
			            for (Integer i=1; i<=12; i++) { //Month Loop Starts here
							...
							if (compareMonths <= calcMonthDiff && compareMonths > 0) {
								System.Debug ('MyDebug - curObject.LumSumType__c = ' + curObject.LumSumType__c);
								System.Debug ('MyDebug - totalMonths = ' + totalMonths);
								// Rent Free Period
			...
			GLXY_LumpSumObligations__c[] objsToDelete = [select id from GLXY_LumpSumObligations__c where LumpSum__c IN :UpdatedOrNewRecordIDs];
			Delete objsToDelete; 
				    
			//Final Obligation amounts by FY Update ...
			Database.UpsertResult[] Upsertresults = Database.Upsert(objLSObligationsByFY, Schema.GLXY_LumpSumObligations__c.FYAndLSId__c, false);
